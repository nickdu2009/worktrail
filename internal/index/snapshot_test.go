...
func TestFreshSourceSnapshotMatchesFreshSnapshotAndClonesSelectedEntries(t *testing.T) {
	...
	if got, want := entryIDs(sourceSnapshot.Entries), []string{"beta", "alpha"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("source entries IDs = %v, want %v", got, want)
	}
	if got, want := recordIDs(sourceSnapshot.Records), []string{"beta", "alpha"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("source record IDs = %v, want %v", got, want)
	}
	if got, want := sourceSnapshot.Records[1].Body, body; got != want {
	...
	if got, want := sourceSnapshot.Entries[1].Tags, []string{"alpha"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("source entry tags changed through selector slice: got %v, want %v", got, want)
	}
	if got, want := sourceSnapshot.Records[1].Entry.Tags, []string{"alpha"}; !reflect.DeepEqual(got, want) {
