...
func TestDetailedSearchRanksAndFiltersLexically(t *testing.T) {
	...
	if entry := hits[0].Entry; entry.ProjectID != "project-a" || entry.TaskID != "task-a" ||
		entry.Visibility != "team" || entry.ExpiresAt.IsZero() {
		t.Fatalf("detailed hit did not hydrate main schema fields: %+v", entry)
	}
}
...
func detailedSearchFixture(t *testing.T) string {
	...
	mustWriteDoc(t, filepath.Join(root, "rules", "entry-a.md"), map[string]any{
		...
		"project_id": "project-a", "task_id": "task-a", "visibility": "team", "expires_at": "2026-06-01T00:00:00Z",
	}, "batchneedle")
