...
worktrail state close --to handoff --next-step "continue the task" "summary, validation, risks, and next step"
...
`worktrail review` now behaves as a semantic draft review surface by default. It still keeps the legacy `candidate` terminology for compatibility, but the default inbox is only pending semantic drafts. Hidden evidence items are reported separately, and operational drafts stay out of the default review table unless you opt into `--all`. Retired `candidate_type=handoff` records are excluded even from `--all`, candidate list/show/diff, and review plans; explicit access returns a migration-required error, and promote/merge remain blocked. Semantic drafts include `source_candidate_ids` details when present, source health warnings, target/duplicate warnings, and a focused `worktrail candidates diff <id>` next step. Use `worktrail review --evidence` for evidence items such as `transcript_notes` and `migration_source`, or `worktrail review --all` when other operational drafts also need inspection.
...
`worktrail context <task>` is the read-only navigation entry for a task. Recovery summaries stay separated by `project_id` and `task_id`; Worktrail never combines state, handoffs, checkpoints, or hook runtime from different tasks into one recovery chain. Context hides pending evidence by default, reports how many evidence candidates are hidden, and skips stale indexed entries instead of surfacing deleted or outdated cached content. Use `worktrail context --evidence <task>` when evidence items need to be included in the pending section, and use `worktrail index diff` or `worktrail index rebuild` when context reports a stale index. Context and search refresh the SQLite index incrementally before reads; `worktrail index rebuild` remains the full-recovery oracle.

`worktrail state close --to handoff --next-step "<action>" "<summary>"` is the primary explicit handoff path when an active state exists: it atomically archives that state and creates a task-bound local handoff under `.worktrail/handoffs/local/`. When no active explicit state exists, use `worktrail handoff create --next-step "<action>" "<summary>"`; use `--complete` instead only when no follow-up remains. Local is the default and is not formal knowledge or a review candidate. To share a durable team record, explicitly run `worktrail handoff publish <local-id>`; publish creates a new immutable record under `.worktrail/handoffs/team/` and never runs `git add`, `git commit`, or `git push`.

Team publish refuses a dirty worktree by default. The exception requires both `--allow-dirty` and `--confirm`, and the published snapshot is marked `code_availability=unavailable`; it does not claim that uncommitted code can be recovered elsewhere. Team records form an immutable per-task DAG through `supersedes`. A single head is linked automatically; multiple heads require an explicit reconciliation publish with `--supersedes`.

`worktrail resume` creates a fresh active state for exactly one task. Its priority is local handoff, team handoff, explicit state, explicit checkpoint, runtime checkpoint, then runtime session. With no selector, exactly one recoverable task may be selected automatically; multiple tasks return an ambiguity error and require `--task-id`, `--task-title`, or `--ref [scope:]kind:id`. Use `worktrail state checkpoint --reason "<reason>"` for an explicit checkpoint; hook checkpoints remain lower-priority degraded runtime artifacts.

Handoff writes are journaled transactions. `worktrail handoff doctor` is read-only and reports malformed records, invalid hashes, local mode/current-record problems, untracked team records, and multiple team heads. `worktrail handoff repair` is dry-run by default; only `worktrail handoff repair --apply --confirm` quarantines malformed local handoffs or performs other local repair, and team records are never edited in place. Local content is redacted or blocked according to the local safety profile; team publish applies the stricter team profile and rejects secret/PII patterns, absolute local paths, raw transcript-style content, and diffs.

Malformed explicit state and hook runtime records use one recovery entry point: run `worktrail doctor recovery` for the read-only state/runtime quarantine plan, then `worktrail doctor recovery --apply --confirm` after review. The recovery command does not accept `--repair`; quarantined files are moved below `.worktrail/runtime/quarantine/state/` or the matching runtime quarantine directory.

Hook runtime records expire after 14 days and recovery reads retain at most the latest five valid records per task. Pruning is explicit and CLI-driven: run `worktrail runtime prune` for the read-only plan, then `worktrail runtime prune --apply --confirm` only after review. Use `worktrail doctor ops status` for read-only journal/lock health and `worktrail doctor ops repair --confirm` only for an explicitly approved replay or recoverable stale-lock repair. Hooks never prune records, create handoffs, publish team records, or change knowledge. Legacy root-level handoffs and retired `candidate_type=handoff` records migrate through a dry-run first:
...
Apply requires both flags. Dry-run fully validates generated V2 metadata, body safety, content hash, and size; invalid legacy IDs, source tools, escaping references, and symlinked references are visible as `invalid` plan items before any backup or target write. The default verified backup is outside the migrated `.worktrail` root under the precisely ignored `/.worktrail-handoff-v2-backups/`; its manifest records the inventory hash, file count, and per-file hashes. Discarded and archived handoff candidates are backed up, migrated with their terminal lifecycle, and removed from the candidate surface. Conflicting targets or backup directories stop the migration, reruns are idempotent, and recovery restores only unchanged destinations from the manifest. A successful CLI apply always rebuilds the project index. `worktrail doctor knowledge` intentionally does not discover handoff candidates; that inventory belongs only to `worktrail migrate handoff-v2`.
