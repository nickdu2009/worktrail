...
8. Prepare candidate commands from each item's plan `commands` array only for `promote`, `merge`, and `discard`. Do not generate a state-changing command for `needs_human_review`.
9. Wait for explicit user confirmation before executing any state-changing command. Confirmation must identify the exact candidate id or accepted batch.
