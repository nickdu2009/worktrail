...
8. Prepare candidate commands from each item's plan `commands` array only for `promote`, `merge`, and `discard`. Preserve any `--scope` flags from the plan commands. Do not generate a state-changing command for `needs_human_review`.
9. Wait for explicit user confirmation before executing any state-changing command. Confirmation must identify the exact candidate id or accepted batch and scope.
10. If `worktrail review` reports an applied candidate with a missing target, explain the two safe paths: `worktrail restore <id>` recreates an accidentally deleted promoted replace target; `worktrail retire <id> --reason <text>` acknowledges an intentionally removed target.
11. Only after confirmation, run the requested non-interactive CLI command: `worktrail promote`, `worktrail merge`, `worktrail discard`, `worktrail restore`, or `worktrail retire`.
