...
- Show only pending semantic drafts (`architecture`, `decision`, `glossary`, `integration`, `lesson`, `project`, `prompt`, `rule`, `validation`, `workflow`) with scope, type, operation, target path, `source_candidate_ids`, source status, redaction status, warnings, reason codes, and suggested commands.
